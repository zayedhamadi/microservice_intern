...
public class ApplyController {
    ...
    @GetMapping("/candidat/{candidatKeycloakId}")
    @PreAuthorize("hasAnyRole('RH','EMPLOYEE')")
    public ResponseEntity<PageResponseDto<ApplicationDto>> getCandidaturesParCandidat(
            @PathVariable String candidatKeycloakId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
            @RequestParam(defaultValue = "dateCandidature") String sortBy,
            @RequestParam(defaultValue = "desc") String sortDir,
            @RequestParam(required = false) ApplicationStatus statut) {

        return ResponseEntity.ok(
                applyService.getCandidaturesParCandidatPaged(candidatKeycloakId, page, size, sortBy, sortDir, statut)
        );
    }
